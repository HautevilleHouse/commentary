...
namespace GeneralSGDMomentDisproofFull
...
def attemptSixteenPrefixRev (n : ℕ) :
    attemptSixteenPrefixIndex n ≃ attemptSixteenPrefixIndex n where
  toFun i := ⟨n - 1 - i.1, by
    have hi : i.1 < n := i.2
    omega⟩
  invFun i := ⟨n - 1 - i.1, by
      ...
      calc
        ...
        _ =
            Finset.sum (Finset.range (n + 1))
              (fun i =>
                α * (1 - α) ^ ((n + 1) - 1 - i) *
                  attemptSixteenCenteredProcessValue i ω) := by
            ...
            calc
              ...
              _ =
                Finset.sum (Finset.range n)
                  (fun j =>
                    α * (1 - α) ^ j *
                      attemptSixteenCenteredNoiseValue (ω (n - 1 - j))) := by
                        simpa using
                          (Finset.sum_range_reflect
                            (fun j ↦ α * (1 - α) ^ j *
                              attemptSixteenCenteredNoiseValue (ω (n - 1 - j))) n)
  rw [hreflect]
  simpa [attemptSixteenPrefixBackwardSum, attemptSixteenPrefixRestrict,
    attemptSixteenPrefixRev_val] using
    (Fin.sum_univ_eq_sum_range
...
end GeneralSGDMomentDisproofFull
