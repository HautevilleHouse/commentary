...
namespace RingelN12StarK25Mathlib
abbrev Edge := Nat × Nat
def edge (a b : Nat) : Edge := if a ≤ b then (a,b) else (b,a)
def allEdges : List Edge :=
  (List.range 25).flatMap (fun i => (List.range (24-i)).map (fun k => (i,i+k+1)))
def copies : List (List Nat) :=
  (List.range 25).map (fun c => c :: (List.range 12).map (fun k => (c+k+1) % 25))
def starEdges : List Nat → List Edge
  | c :: rest => rest.map (edge c)
  | _ => []
def usedEdges : List Edge := (copies.map starEdges).flatten
def validStar (p : List Nat) : Bool := p.length = 13 && p.Nodup

-- Prefer native_decide for large finite checks (kernel decide OOMs on K_25).
